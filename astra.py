...
## Python 2 compatibility aliases (many plugins still use py2 helpers).
basestring = str
unichr = chr
file = open
...
def execfile(filename, gl=None):
        exec(compile(open(filename, encoding='utf-8').read(), filename, 'exec'), gl if gl is not None else globals())
...
def register_command_handler(instance, command, category = [], access = 0, desc = None, syntax = None, examples = []):
        pass  # py3: command is str
        if not command in COMMSTAT:
...
def get_Thr_id(handler):
        INFO['thr'] += 1
        return '%s-%d' % (handler.__name__, INFO['thr'])
...
def call_join_handlers(conf, nick, afl, role, status = None, text = None):
        for handler in JOIN_HANDLERS:
                if handler.__name__ == "logWriteJoined":
...
def call_command_handlers(command, typ, source, body, callee):
   confa = source[1]
   if confa not in AFOOLS.keys():
      if confa not in PRIVATE_TYPE.keys():
         call_command_handlers1(command, typ, source, body, callee)
      else:
         if typ != 'private':
            reply(typ,source,u'---')
         call_command_handlers3(command, 'private', source, body)
   else:
      fools = random.choice(['1','2','3','4'])
      if fools == '2':
         reply(typ,source,random.choice([u'Не могу.....у меня мизофобия',u':-p',u'Рожа не треснет?!']))
      else:
         call_command_handlers1(command, typ, source, body, callee)

def call_command_handlers3(command, typ, source, body):
        ...
        if command in COMMAND_HANDLERS:
                if has_access(source[0], real_access, source[1]):
                        ...
                        if source[1] not in POL_SEX.keys():
                                reply(typ, source, random.choice([u"Недостаточный доступ.",u"Ага, щаз прям. Разбежалась :-)",u"Неа ]:->",u"Низя тебе это трогать *GIRL_DEVIL*",u"Еще раз и Бан получишь *PUNISHED*",u"*nono*",u"нихачу",u"а еще разок :-)"]))
                        else:
                                reply(typ, source, u"Недостаточный доступ.")

POL_SEX = {}

def call_command_handlers2(command, typ, source, body):
        real_access = COMMANDS[command]['access']
        if real_access <= 0:
                real_access = COMMANDS[command]['access']
        if command in COMMAND_HANDLERS:
                if has_access(source[0], real_access, source[1]):
                        ...
                        if jid not in COMMSTAT[command]['users']:
                         ...
                         return#if source[1] not in POL_SEX.keys():
#	 	 		reply(typ, source, random.choice([u"Недостаточный доступ.",u"Ага, щаз прям. Разбежалась :-)",u"Неа ]:->",u"Низя тебе это трогать *GIRL_DEVIL*",u"Еще раз и Бан получишь *PUNISHED*",u"*nono*",u"нихачу",u"а еще разок :-)"]))
#	 	 	else:
#	 	 	 	reply(typ, source, u"Недостаточный доступ.")

def call_command_handlers1(command, typ, source, body, callee):
...
def load_plugins():
        ...
        for Plugin in os.listdir(PLUGIN_DIR):
                if Plugin.endswith('.py'):
                        ...
                        try:
                                data = open(filename, encoding='utf-8', errors='replace').read(20)
                        except:
                                data = str()
                        Plug = Plugin.split('.')
...
read_link = lambda link: decode_page(urlopen(link).read())
...
def handler_jid(instance):
        ...
        chat = List[0].lower()
        if (len(List) == 2) and chat in GROUPCHATS:
                if List[1] in GROUPCHATS[chat]:
...
def command_Prefix(conf, command):
        if command in (u'хелп', u'?', u'помощь', u'команды', u'тест', u'префикс'):
                return command
...
def user_level(source, conf):
        ...
        elif conf in CONFACCESS and jid in CONFACCESS[conf]:
                level = CONFACCESS[conf][jid]
        elif conf in ACCBYCONF and jid in ACCBYCONF[conf]:
...
def reply(ltype, source, body):
        ...
        if ltype == 'public':
                ...
                if jid in A_NAME.keys():
                        body = '%s, %s' % (str.join('',A_NAME[jid]), body)
                        msg(source[1], body)
                else:
...
def MESSAGE_PROCESSING(client, stanza):
        ...
        if instance in MACROS.macrolist.keys():
                cmds = list(MACROS.gmacrolist) + list(MACROS.macrolist[instance])
        else:
                cmds = list(MACROS.gmacrolist)
        if not cbody:
                raise xmpp.NodeProcessed()
        command = cbody.split()[0].lower()
        if instance in PREFIX and rcmd not in cmds:
                 jid = handler_jid(instance)
                 if jid != u'vlasov61292@jabber.ru':
                          NotPfx = Prefix_state(body, bot_nick)
                          if NotPfx or ltype == 'chat':
                                   if not command in COMMANDS and command[:1] in ("!", "@", "#", ".", "*", "+", "`"):
                                           command = command[1:]
                          else:
...
def calc_acc(conf, jid, role):
        if not jid in GLOBACCESS:
                if not (conf in CONFACCESS and jid in CONFACCESS[conf]):
        ...
        if conf in GROUPCHATS:
                ...
                if Ptype == 'unavailable':
                        ...
                        if scode in ('301', '307') and nick == handler_botnick(conf):
                                ...
                                delivery(u'Меня %s в "%s" и я оттуда вышла.' % (text, conf))
                                raise xmpp.NodeProcessed()
                        elif scode == '303':
                                full_jid = stanza.getJid()
                                if not full_jid:
                                        full_jid = str(fromjid)
                                        jid = str(fromjid)
                                else:
                        ...
                        if ecode:
                                if ecode == '409':
                                        ...
                                        try:
                                                ThrName = "rejoin-%s" % (conf)
                                                if ThrName not in [x.name for x in list(threading._active.values())]:
...
def IQ_PROCESSING(client, iq):
        ...
        if iq.getType() == "get":
                ...
                if nsType == xmpp.NS_VERSION:
                        ...
                        ids = [{'category': 'client', 'type': 'user', 'name': 'Astra'}]
...
def proces_igra():
   while START_IGRA:
      time.sleep(180)
      for x in list(START_JID):
         WOOD[x] += 1 * int(WOOD_COL[x])
         with open('dynamic/wood.txt', 'w') as fp: fp.write(str(WOOD))
         STONE[x] += 1 * int(STONE_COL[x])
         with open('dynamic/stone.txt', 'w') as fp: fp.write(str(STONE))
         IRON[x] += 1 * int(IRON_COL[x])
         with open('dynamic/iron.txt', 'w') as fp: fp.write(str(IRON))
         EAT[x] += 1 * int(EAT_COL[x])
         with open('dynamic/eat.txt', 'w') as fp: fp.write(str(EAT))
...
def main():
        ...
        call_stage_init(0)
        join_chats()
        threading.Thread(None, proces_igra, 'procrs_igra').start()
        Print('\n\nAstra is ready to work!\n\n', color3)
