...
class Macros:
	...
	def remove(self, mapee, conf = None):
		if conf:
			if mapee in self.macrolist[conf]:
				del self.macrolist[conf][mapee]
			if mapee in self.accesslist[conf]:
				del self.accesslist[conf][mapee]
		else:
			if mapee in self.gmacrolist:
				del self.gmacrolist[mapee]
			if mapee in self.gaccesslist:
	...
	def get_access(self, macro, conf):
		if conf in self.macrolist:
			if macro in self.accesslist[conf]:
				return self.accesslist[conf][macro]
		if macro in self.gaccesslist:
