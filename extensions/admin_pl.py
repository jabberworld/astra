...
def handler_admin_join1(type, source, body):
        if body:
                list = body.split()
...
def handler_admin_rejoin1(type, source, body):
        if body:
...
def handler_admin_leave1(type, source, body):
        if body:
                list = body.split()
                conf = list[0].strip().lower()
                if len(list) == 1:
...
def handler_admin_restart1(type, source, reason):
...
def handler_admin_exit1(type, source, reason):
...
register_command_handler(handler_admin_join1, '!джойн', [], 80, '', '', [''])
register_command_handler(handler_admin_rejoin1, '!реджойн', [], 80, '', '', [''])
register_command_handler(handler_admin_leave1, '!свал', [], 80, '', '', [''])
register_command_handler(handler_admin_restart1, '!рестарт', [], 100, '', '', [''])
register_command_handler(handler_admin_exit1, '!выкл', [], 100, '', '', [''])
#command_handler(handler_error_stat, 100, "admin")
#command_handler(handler_timeup_info, 10, "admin")
#command_handler(handler_botup_info, 10, "admin")
#command_handler(handler_command_stat, 10, "admin")
#command_handler(crashReport_cfg, 100, "admin")


#egister_stage1_init(load_conf_prefix)
