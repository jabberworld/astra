...
def alias_exec_add_join(t, s, p):
    ...
    for x in COMMAND_HANDLERS:
        if p.count(' '+x+' ') or p.count('#'+x+' ') and len(x)>2:
            ...
            if not has_access(s[0], real_access, s[1]):
    ...
    if int(user_level(s[0], s[1]))<100:
        ...
        for x in [x for x in GLOBACCESS.keys() if GLOBACCESS[x]==100]:
            time.sleep(1)
            delivery(u'Новый алиас поступил на проверку от '+s[1]+'/'+s[2]+u', id '+str(n)+u'.\nДля активации напишите id алиаса!\n'+org)
        return
    else:
...
def alias_exec_add_msg(t, s, p):
    ...
    cnt = 0
    if not p:
        if not s[1] in ALIASS.keys() or not ALIASS[s[1]]:
            reply(t, s, u'Нет алиасов!')
            return
        reply(t, s, ';\n'.join([str(x)+') '+ALIASS[s[1]][x] for x in ALIASS[s[1]]]))
        return
    if p.isdigit() and s[1] in ALIASS.keys():
    ...
    if int(user_level(s[0], s[1]))<100:
        ...
        for x in [x for x in GLOBACCESS.keys() if GLOBACCESS[x]==100]:
            delivery(u'Новый алиас поступил на проверку с комнаты '+s[1]+u', id '+str(n)+u'.\nДля активации напишите id алиаса!\n'+org)
        return
    else:
        if not s[1] in ALIASS.keys(): ALIASS[s[1]]={}
        n = len(ALIASS[s[1]])+1
        ALIASS[s[1]][n]={}
        ALIASS[s[1]][n]=p
...
def alias_msg(r, t, s, p):
    if not s[1] in ALIASS.keys(): return
    if s[2] == handler_botnick(s[1]): return
    if p.count(' ') and p.split()[0].lower() in COMMANDS.keys(): return
    userjid = handler_jid(s)
    ...
    nick = s[2]
    chat = s[1]
    for x in ALIASS[s[1]].keys():
        i=ALIASS[s[1]][x].replace('$p0',p0).replace('$p1',p1).replace('$lastmsg',str(lastmsg)).replace('$nick',nick).replace('$chat',chat).replace('$userjid',userjid).replace('$serv',serv).replace('$p',p)
        try: exec(i)
        except Exception as err:
            try: delivery(u'Зарегестрировано исключение при выполнении '+ALIASS[s[1]][x]+':\n'+err)
            except: pass
            del ALIASS[s[1]][x]

def alias_join(chat, nick, role, aff):
    t='public'
    s=[chat+'/'+nick,chat,nick]
    if not chat in GROUPCHATS: return
    if not chat in ALIAS_JOIN: return
    if nick == handler_botnick(chat): return
    userjid = handler_jid(chat+'/'+nick)
    serv, timej = '', 0
    try: serv = userjid.split('@')[1]
    except: pass
    ...
    for x in ALIAS_JOIN[chat].keys():
        ...
        try: exec(i)
            ...
            try: delivery(u'Зарегестрировано исключение при выполнении '+ALIAS_JOIN[chat][x]+':\n'+err)
            except: pass
...
def alias_adm_check(r, t, s, p):
    ...
    if jid in [x for x in GLOBACCESS.keys() if GLOBACCESS[x]==100] and p.isdigit() and int(p)>19:
        ...
        if ak:
            if not chat in ALIASS.keys(): ALIAS[chat] = {}
            
            n = len(ALIASS[chat])+1
            
            if n in ALIASS[chat].keys(): n = random.randrange(99, 9999)
