...
def handler_presence_nickcommand(prs):
        ...
        if groupchat in GROUPCHATS:
                ...
                if not nick or not nick.split():
                        return
                nicksource=nick.split()[0].strip().lower()
                if nicksource in (list(COMMANDS) + list(MACROS.gmacrolist) + list(MACROS.macrolist[groupchat])):
                        handler_kick(groupchat, nick, handler_botnick(groupchat)+u' :your nickname is invalid here')
