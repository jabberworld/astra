...
def millionaire_load(groupchat,jid):
        global MILLIONAIRE
        if not groupchat in MILLIONAIRE:
...
def millionaire_start(type, source, parameters):
        ...
        if not 'stage' in MILLIONAIRE[groupchat][jid]:
...
def millionaire_stop(type, source, parameters):
        ...
        if type == 'private':
                ...
                jid = 'all'
        if source[1] in MILLIONAIRE and jid in MILLIONAIRE[source[1]]:
...
def millionaire_summary(type, source, parameters):
        ...
        if parameters:
                if parameters in GROUPCHATS[source[1]]:
                        jid = handler_jid(source[1] + '/' + parameters)
                elif parameters.count('@') and parameters.count('.'):
                        jid=parameters
                else:
                        reply(type, source, u'Я такого не знаю.')
                        return
                if not jid in MILLIONAIRE[groupchat]:
                        millionaire_load(source[1],jid)
                if 'summary' in MILLIONAIRE[groupchat][jid]:
                ...
                if type == 'private':
                        ...
                        if 'summary' in MILLIONAIRE[groupchat][jid] and 'summary' in MILLIONAIRE[groupchat]['all']:
...
def handler_millionaire_chat(raw, type, source, parameters):
        ...
        if not parameters.isdigit():
                return
        if not source[1] in MILLIONAIRE:
                return
        if jid in MILLIONAIRE[source[1]] and 'answer' in MILLIONAIRE[source[1]][jid]:
...
register_command_handler(millionaire_call, '!миллионер', ['игры','все'], 10, 'Игра "О,счастливчик! \n(При игре в привате начисление очков идет на счет вашего JID, а в общем чате - на счет конфы.)\nПодробности в самой команде. \nОтвечать на вопросы нужно цифрой!\nP.S.: деньги не выплчиваются, не перечисляются и вобще они не существуют ;)', '!миллионер [<параметр>]', ['!миллионер','!миллионер старт','!подсказка 2'])
register_command_handler(millionaire_hint, '!подсказка', ['игры','все'], 10, 'Подсказки для игры "О,счастливчик!', '!подсказка <число>', ['!подсказка','!подсказка 1'])
