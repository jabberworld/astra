...
#    if p.count(' ')>1:
#        return
    if s[1] in GROUPCHATS.keys() and t != 'private':
        bn = handler_botnick(s[1])
        if not p.count(bn):
            return
    ...
    if not sp or len(sp)>1:
        return
    if sp[0].lower() in [u'свал',u'выкл',u'рестарт']: return
#    reply(t, s, u'Возможно, вы имели в виду -\n'+''.join(sp)+u' ?')
    ...
    if s[1] in COMMOFF and command in COMMOFF[s[1]]:
        ...
        call_command_handlers2(command, t, s, unicode(c))
        INFO['cmd'] += 1
...
def cmd_repl(raw, type, source, body):
        try:
                if not body:
                        return
                if len(body)>50:
                ...
                if command in COMMANDS:
                        if source[1] in COMMOFF and command in COMMOFF[source[1]]:
                                ...
                                reply(type, source, u'Команда распознана как:\n'+rep)
                                time.sleep(1.5)
                                call_command_handlers2(command, type, source, unicode(c))
                                INFO['cmd'] += 1
