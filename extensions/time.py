        ...
        if source[1] in GROUPCHATS:
                if body:
                        ...
                        if body in GROUPCHATS[source[1]]:
                                if 'timexep' in GROUPCHATS[source[1]][body]:
                        ...
                        jid = source[0]
                        if 'timexep' in GROUPCHATS[source[1]][source[2]]:
...
def gettime_xep0090_answer(coze, res, nick, type, source):
                if res:
                        if res.getType() == 'error':
                                ...
                                if time:
                                        if nick:
                                                reply(type, source, u'У "%s" на солнечных *TIME* сейчас %s' % (nick, time))
                                        else:
                                                reply(type, source, u'У тебя на солнечных *TIME* сейчас %s' % (time))
                ...
                if res:
                        if res.getType() == 'error':
                                ...
                                if tzo and utc:
                                        ...
                                        if nick:
                                                reply(type, source, u'У "%s" на солнечных *TIME* сейчас %s (%s)' % (nick, time, date))
                                        else:
                                                reply(type, source, u'У тебя на солнечных *TIME* сейчас %s (%s)' % (time, date))
                                else:
