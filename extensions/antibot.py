...
BOTS = 'Jaskier/BlackSmith/AnGeL/ταλιςμαη/talisman/Talisman/Neutron/Юта/fatal/Åmulet/lesman/VIKA/iBot'
...
def antibot_leave(conf):
        if conf in GROUPCHATS and conf in ANTIBOT_LIST:
                if conf in EXCEPTIONS:
                        ...
                        delivery(u'В "%s" было слишком много ботов и я ушла!' % (conf))
                        ...
                        if conf.count('@conference.') and conf.count('.') >= 2 and conf in GROUPCHATS:
                                ...
                                if check == '+':
                                        if conf not in EXCEPTIONS:
                                                ...
                                                repl = u'добавлено "%s" в список исключений.' % (conf)
                                        else:
                                        ...
                                        if conf in EXCEPTIONS:
                                                ...
                                                write_file(EXCEPT_LIST, str(EXCEPTIONS))
                                                repl = u'удалено "%s" из списка исключений.' % (conf)
                                        else:
...
def exceptions_cleanup():
        for x in EXCEPTIONS:
                if not GROUPCHATS.get(x):
                        EXCEPTIONS.remove(x)
        write_file(EXCEPT_LIST + "_", repr(EXCEPTIONS))
...
command_handler(handler_antibot_exceptions, 80, "antibot")
